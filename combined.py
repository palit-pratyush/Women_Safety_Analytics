...
# Custom DepthwiseConv2D layer to handle unrecognized arguments in the models
...
gender_labels = open("labels_gender.txt", "r").readlines()
    ...
    while True:
        ...
        for i in range(0, detections.shape[2]):
            ...
            if confidence > 0.5:
                ...
                gender_class_name = gender_labels[gender_index].strip()
                gender_confidence = gender_prediction[0][gender_index]
                ...
                label = f"{gender_class_name}: {gender_confidence*100:.2f}%"
        ...
        violence_frame = cv2.resize(frame, (224, 224), interpolation=cv2.INTER_AREA)
        violence_array = np.asarray(violence_frame, dtype=np.float32).reshape(1, 224, 224, 3)
        violence_array = (violence_array / 127.5) - 1

        # Predict violence
        violence_prediction = violence_model.predict(violence_array)
        violence_index = np.argmax(violence_prediction)
        violence_class_name = violence_labels[violence_index].strip()
        violence_confidence = violence_prediction[0][violence_index]

        # Display violence prediction on the frame
        violence_text = f"Violence: {violence_class_name} ({violence_confidence*100:.2f}%)"
        cv2.putText(frame_with_pose, violence_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
